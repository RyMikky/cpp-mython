...
namespace parse {
    ...
    namespace {
        ...
        void TestSimpleAssignmentTwo() {
            istringstream input("'; '"s);
            Lexer lexer(input);

            ASSERT_EQUAL(lexer.CurrentToken(), Token(token_type::String{ "; "s }));
            ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Newline{}));
            ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Eof{}));
...
#)"s);
        ...
        void TestYandexSix() {
            ...
            ASSERT_EQUAL(lexer.CurrentToken(), Token(token_type::String{ "\"'\t\n"s }));

        }

        void TestYandexFourt() {

            istringstream input(R"(return "Bypass/base operation interface class")"s);
            Lexer lexer(input);

            ASSERT_EQUAL(lexer.CurrentToken(), Token(token_type::Return{}));
            ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String{"Bypass/base operation interface class"}));
    ...
    void RunOpenLexerTests(TestRunner& tr) {
        ...
        RUN_TEST(tr, parse::TestSimpleAssignmentTwo);
        ...
        RUN_TEST(tr, parse::TestYandexSix);
        RUN_TEST(tr, parse::TestYandexFourt);
    }
